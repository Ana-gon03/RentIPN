    ...
    <>
      ...
      {menuAbierto && (
        <div className="nav-mobile-menu">
          {LINKS.map(({ href, label }) => (
            <a key={href} href={href} className="nav-mobile-link" onClick={cerrarMenu}>
              {label}
            </a>
          ))}
          <Link to="/usuarios/inicio-sesion" className="nav-mobile-link" onClick={cerrarMenu}>
            Iniciar Sesión
          </Link>
          <Link to="/registro" className="nav-mobile-link" onClick={cerrarMenu}
            style={{ color: "#534AB7", fontWeight: 700 }}>
            Registrarse
          </Link>
        </div>
      )}
